  ...
  const mappedData = models.slice(0).reverse().map((model:any) => {
    ...
    return (
      <TableRow key={model._id}>
        ...
        <TableCell align='center' className='actions'><Link className='link' to={`/admin/addModel/update/${model._id}?isEdit=true`}><Button className='btn' variant='contained' size='small' style={{ backgroundColor: 'lightseagreen' }}><Edit className='icon' /></Button></Link>
